...
class CommentForm(forms.Form):
    ...
    email = forms.EmailField(widget=forms.TextInput(
        attrs={'id': 'email', 'type': 'email', 'class': 'comment_input', 'required': 'required', 'size': '25',
               'tabindex': '2'}), max_length=50, error_messages={'required': 'email不能为空', })
    url = forms.URLField(widget=forms.TextInput(
        attrs={'id': 'url', 'type': 'url', 'class': 'comment_input', 'size': '25', 'tabindex': '3'}), max_length=120,
                         required=False)
    comment = forms.CharField(widget=forms.Textarea(
        attrs={'id': 'comment', 'class': 'message_input', 'required': 'reuqired', 'cols': '25', 'rows': '5',
               'tabindex': '4'}), error_messages={'required': '评论不能为空'})
